...
class ProductTemplate(models.Model):
    ...
    rent_ok = fields.Boolean(
        string="Can be Loaner",
        help="Allow loaner of this product.")
    qty_in_rent = fields.Float("Quantity currently in loaner", compute='_get_qty_in_rent')
    is_can_be_assets = fields.Boolean(string='Can be Assets')
    # product_part = fields.Char(String='Product Part No.')

    @api.onchange('tracking')
    def onchange_tracking(self):
        if self.tracking == 'serial':
            self.is_can_be_assets = True
        else:
            self.is_can_be_assets = False

    @api.onchange('is_can_be_assets')
    def onchange_is_can_be_assets(self):
        if self.is_can_be_assets:
    ...
    def action_view_rentals(self):
        """Access Gantt view of loaner (sale.rental.schedule), filtered on variants of the current template."""
        return {
            "type": "ir.actions.act_window",
            "name": _("Scheduled Loaner"),
    ...
    def name_get(self):
        ...
        if not self._context.get('rental_products'):
            return res_names
        return [
            (res[0], self.browse(res[0]).rent_ok and _("%s (Loaner)", res[1]) or res[1])
