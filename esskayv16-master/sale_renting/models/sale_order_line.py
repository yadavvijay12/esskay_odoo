...
class SaleOrderLine(models.Model):
    ...
    temporal_type = fields.Selection(selection_add=[('loaner', 'Loaner')])
    warranty_select = fields.Selection([("repair_warranty", "Repair Warranty"), ("extended_warranty", "Extended Warranty")], string="Warranty")
    is_warranty_product = fields.Boolean('Warranty Products')
    asset_id = fields.Many2one('stock.lot', string="Asset", copy=False, domain="[('product_id', '=',product_id)]")
    ...
    def _onchange_product_id_warning(self):
        ...
        if self.product_id and self.product_id.product_tmpl_id.order_type in ('extended_warranty', 'repair_warranty'):
            self.is_warranty_product = True
            if self.product_id.product_tmpl_id.order_type == 'extended_warranty':
                self.warranty_select = 'extended_warranty'
            else:
                self.warranty_select = 'repair_warranty'
    ...
    def _prepare_new_rental_procurement_values(self, group=False):
        vals = {
            ...
            # "route_ids": self.env['stock.route'].browse(5),
    ...
    def _run_rental_procurement(self, vals):
        ...
        procurements = [
            self.env["procurement.group"].Procurement(
                ...
                # self.env['stock.location'].browse(25),
                self.name,
                self.order_id.name,
    ...
    def _action_launch_stock_rule(self, previous_product_uom_qty=False):
        errors = []
        if self.filtered(lambda x:x.is_rental ==True):
            for line in self:
                if line.is_rental and line.product_id.rent_ok:
