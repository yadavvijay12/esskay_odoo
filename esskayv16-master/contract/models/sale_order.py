...
class SaleQuotationCustom(models.Model):
    ...
    def action_confirm_warranty(self):
        for rec in self.order_line.filtered(
                lambda p: p.service_product_id.order_type in ('repair_warranty', 'extended_warranty')):
            asset_id = rec.asset_id
            # Warranty End Date based on product template warranty terms
            interval = int(rec.service_product_id.term_type_id.number_days)
            if rec.service_product_id.term_type_id.interval_type == "daily":
                interval_unit = relativedelta(days=interval)
            elif rec.service_product_id.term_type_id.interval_type == "monthly":
            ...
            if rec.service_product_id.term_type_id.warranty_date_based == 'installation_date':
                if not asset_id.warranty_end_date:
                    ...
                    values = {
                            ...
                            'warranty_type': rec.service_product_id.order_type,
