...
class SpareRequest(models.Model):
    ...
    picking_type_id = fields.Many2one('stock.picking.type', 'Picking Type', required=True,
                                      default=_get_default_picking_type, copy=False,
                                      domain="[('code', '=', 'internal'), ('company_id', '=', company_id)]")
    state = fields.Selection(
        [('draft', 'Draft'), ('external_stock_check', 'External Stock Check'), ('available', 'Available'),
    ...
    availability_state = fields.Selection(
        [('draft', 'Draft'), ('available', 'Available'), ('not_available', 'Not Available'),
         ('partially_available', 'Partially Available')], default='draft',
        copy=False, index=True, readonly=True)
    ...
    # spare_id = fields.Many2one("multi.approval", string="Spares")
    ...
    def action_check_availability(self):
        ...
        if self.spare_request_line:
            if all(x.qty_available >= x.product_uom_qty for x in self.spare_request_line):
                ...
                self.availability_state = 'not_available'
            else:
                self.state = 'partially_available'
                self.availability_state = 'partially_available'
    ...
    def _compute_totals(self):
        for rec in self:
            ...
            elif rec.spare_request_line and all(move.state == 'done' for move in rec.spare_request_line) and sum(
                    rec.spare_request_line.mapped('product_uom_qty')) > 0.00:
                self.state = 'issued'
            else:
                print('')
    ...
    def action_open_approval_wizard(self):
        # Purpose is to get the approval selection to send for an approval
        self.ensure_one()
        if self.sudo().parent_ticket_id.parent_configuration_id.is_approval_required_spares or self.sudo().child_ticket_id.child_configuration_id.is_approval_required_spares:
            if self.sudo().parent_ticket_id.parent_configuration_id.is_approval_required_spares:
                ticket_id = self.sudo().parent_ticket_id.parent_configuration_id.id
                ticket_model = 'parent.ticket.configuration'
            elif self.sudo().child_ticket_id.child_configuration_id.is_approval_required_spares:
                ticket_id = self.sudo().child_ticket_id.child_configuration_id.id
                ticket_model = 'child.ticket.configuration'
            return {
                'type': 'ir.actions.act_window',
                'name': 'Send for Approvals',
                'view_mode': 'form',
                'res_model': 'spare.approval',
                'context': {'default_is_spare_approval': True, 'default_spare_id': self.id, 'create': False,
                            'ticket_id': ticket_id, 'ticket_model': ticket_model},
                'target': 'new'
            }
        else:
            self.state = 'approved'
    ...
    def action_request_approval(self):
        ...
        if self.sudo().parent_ticket_id:
            if self.sudo().parent_ticket_id.parent_configuration_id.is_approval_required_spares and self.sudo().parent_ticket_id.parent_configuration_id.spares_approval_team_ids:
                ...
                if self.sudo().parent_ticket_id.attachment_ids:
                    for attachment in self.sudo().parent_ticket_id.attachment_ids:
                        ir_values = {
                            'name': str(attachment.name) + "Copy",
                        ...
                        requests.attachment_ids = [(6, 0, [report_attachment_id.id])]
        else:
            if self.sudo().child_ticket_id.child_configuration_id.is_approval_required_spares or self.sudo().child_ticket_id.parent_ticket_id.parent_configuration_id.is_spares:
                approval_id = None
                if self.sudo().child_ticket_id.child_configuration_id.is_approval_required_spares:
                    approval_id = self.sudo().child_ticket_id.child_configuration_id.spares_approval_team_ids
                    approval_type_id = self.sudo().child_ticket_id.child_configuration_id.spares_approval_team_ids[
                        -1].id
                if approval_id:
                    request = {
                        'name': self.name,
                    ...
                    requests = self.env['multi.approval'].create(request)
                    if self.sudo().child_ticket_id.child_ticket_attachments:
                        for attachment in self.sudo().child_ticket_id.child_ticket_attachments:
                            ir_values = {
                                'name': str(attachment.name) + "Copy",
                                'type': attachment.type,
                                'datas': attachment.datas,
        ...
        if requests:
            ...
            mail = ''
            if self.sudo().parent_ticket_id:
                if self.sudo().parent_ticket_id and self.sudo().parent_ticket_id.parent_configuration_id.is_approval_required_spares and self.sudo().parent_ticket_id.parent_configuration_id.spares_approval_team_ids:
                    for approval_mail_id in self.sudo().parent_ticket_id.parent_configuration_id.spares_approval_team_ids:
                        for approvers in approval_mail_id.line_ids:
                            if mail:
                                mail += ',' + approvers.user_id.login
                            else:
                                mail = approvers.user_id.login
            # Take the approval master and send mail to them for an approval
            elif self.sudo().child_ticket_id.child_configuration_id.is_spares:
                for approval_mail_id in self.sudo().child_ticket_id.child_configuration_id.spares_approval_team_ids:
                    for approvers in approval_mail_id.line_ids:
                        if mail:
                            mail += ',' + approvers.user_id.login
                        else:
                            mail = approvers.user_id.login
            if template_id:
                template_id.with_context(rec_url=base_url, email_to=mail, request_name=requests.name).sudo().send_mail(
    ...
    def create(self, values):
        ...
        if not values.get('name'):
            sequence = self.env['ir.sequence'].next_by_code('spare.request')
        ...
        if 'state' in values and self.parent_ticket_id or self.child_ticket_id:
            ...
            if self.parent_ticket_id.parent_configuration_id and self.parent_ticket_id.parent_configuration_id.is_spares:
                ...
                if self.parent_ticket_id.parent_configuration_id.alert_spares == 'confirm' and self.state == 'confirmed':
                    ...
                    send = True
                elif self.parent_ticket_id.parent_configuration_id.alert_spares == 'both' and self.state in [
                    'confirmed', 'issued']:
                    send = True
                ...
                if team_alert_template_id and send:
                    teams = self.env['hr.employee'].sudo().search([('job_id', 'in',
                                                                    self.parent_ticket_id.parent_configuration_id.alert_spares_approval_team_ids.ids)]).mapped(
                        'work_email')
                    email += ', '.join(teams)
            ...
            if self.child_ticket_id.child_configuration_id and self.child_ticket_id.child_configuration_id.is_spares:
                ...
                if self.child_ticket_id.child_configuration_id.alert_spares == 'confirm' and self.state == 'confirmed':
                    ...
                    send = True
                elif self.child_ticket_id.child_configuration_id.alert_spares == 'both' and self.state in ['confirmed',
                                                                                                           'issued']:
                    send = True
                ...
                if team_alert_template_id and send:
                    teams = self.env['hr.employee'].sudo().search([('job_id', 'in',
                                                                    self.child_ticket_id.child_configuration_id.alert_spares_approval_team_ids.ids)]).mapped(
                        'work_email')
                    email += ', '.join(teams)
    ...
    def action_approve(self):
        ...
        for lines in self.spare_request_line:
            ...
            move_vals.update({
                ...
                'product_uom_qty': lines.qty_available if lines.spare_request_id.availability_state == 'partially_available' else lines.product_uom_qty,
        ...
        if stock_picking_id:
            action = {
                'name': _('Release Spare Parts'),
                'type': 'ir.actions.act_window',
                'res_model': 'stock.picking',
                'target': 'current',
            }
            action['res_id'] = stock_picking_id.id
    ...
    def show_quality_checks(self):
        action = {
            'name': _('Release Spare Part(s)'),
            'type': 'ir.actions.act_window',
            'res_model': 'stock.picking',
            'target': 'current',
        }
        stock_picking_id_ids = self.env['stock.picking'].search([('spare_request', '=', self.id)])
        if len(stock_picking_id_ids) == 1:
            action['res_id'] = stock_picking_id_ids.ids[0]
            action['view_mode'] = 'form'
            action['views'] = [[self.env.ref('stock.view_picking_form').id, 'form']]
        else:
            ...
            action['domain'] = [('id', 'in', stock_picking_id_ids.ids)]
        return action
...
class SpareRequestLines(models.Model):
    ...
    remaining_qty = fields.Float("Remaining Qty", compute='_compute_remaining_qty',compute_sudo=True)
    qty_available = fields.Float('Available Qty', compute='_show_stock_qty',compute_sudo=True)
    ...
    source_location_id = fields.Many2one("stock.location", string="Source", domain=[('usage', '=', 'internal')],
                                         related='spare_request_id.picking_type_id.default_location_src_id')
    destination_location_id = fields.Many2one("stock.location", string="Destination",
                                              domain=[('usage', '=', 'internal')], required=True)
    ...
    def _show_stock_qty(self):
        for rec in self:
            stock_qty = self.env['stock.quant'].search(
                [('location_id', '=', rec.source_location_id.id), ('product_id', '=', rec.product_id.id)])
            if rec.product_id:
                rec.update({
...
class MultiApproval(models.Model):
    ...
    spare_request_line_ids = fields.Many2many(comodel_name="spare.request.lines", relation='spare_request_line_rel',compute='_compute_spare_check',
                                              string="Spares")

    def _compute_spare_check(self):
        for rec in self:
            if rec.spare_request_id:
                spare_request_line_ids = self.env['spare.request'].search(
                    [('spare_request_line', '=', rec.spare_request_id.id)])
                if spare_request_line_ids:
                    rec.spare_request_line_ids = spare_request_line_ids.ids
                else:
                    rec.spare_request_line_ids = None
            else:
                rec.spare_request_line_ids = None

    @api.depends('parent_ticket_id', 'child_ticket_id', 'service_request_id', 'spare_request_id')
    def _get_customer_details(self):
        res = super(MultiApproval, self)._get_customer_details()
        for record in self:
            if record.spare_request_id:
                record.partner_id = record.spare_request_id.child_ticket_id.partner_id.id
                # record.customer_type = record.spare_request_id.child_ticket_id.partner_id.customer_type.id if record.spare_request_id.child_ticket_id.partner_id.customer_type else None
                record.cmc_status = record.spare_request_id.child_ticket_id.cmc_status
                record.amc_status = record.spare_request_id.child_ticket_id.amc_status
                record.action_taken_at_site = record.spare_request_id.child_ticket_id.action_taken_at_site
