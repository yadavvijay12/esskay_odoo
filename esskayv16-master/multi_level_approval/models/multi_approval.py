...
class MultiApproval(models.Model):
    ...
    is_pic = fields.Boolean(compute='_check_pic', compute_sudo=True)
    follower = fields.Text('Following Users', default='[]', copy=False)

    pic_group_ids = fields.Many2many('res.users', string='Approvers', compute='_get_approvals', compute_sudo=True)

    # copy the idea of hr_expense
    attachment_number = fields.Integer(
        'Number of Attachments', compute='_compute_attachment_number', compute_sudo=True)
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company)
    reason = fields.Text('Reason')
    approve_date = fields.Datetime(string="Approved Date", compute='action_approved_approvers')

    @api.depends('state')
    def action_approved_approvers(self):
        for rec in self:
            if rec.state == 'Approved':
                rec.sudo().approve_date = rec.write_date
            else:
                rec.sudo().approve_date = False
    ...
    def _get_approvals(self):
        if self.line_ids:
            ...
            res = [self.line_ids[i: j] for i, j in
                   zip([0] + idx_list, idx_list + ([size] if idx_list[-1] != size else [])) if j > 0]
            required_ids = []
            for record in res:
                for each in record:
    ...
    def action_approve(self):
        ...
        for rec in recs:
            ...
            if other_lines:
                ...
                res = [other_lines[i: j] for i, j in
                       zip([0] + idx_list, idx_list + ([size] if idx_list[-1] != size else [])) if j > 0]
            if not other_lines:
                ...
                base_url = '/web#id=%d&action=%r&model=multi.approval&view_type=form' % (
                    next_line.approval_id.id, action_id.id)
                if template_id:
                    template_id.with_context(rec_url=base_url, email_to=next_line.user_id.partner_id.email,
                                             next_approver_name=next_line.user_id.name).sudo().send_mail(
                        next_line.approval_id.id, force_send=True)
    ...
    def _create_approval_lines(self):
        ...
        for r in self:
            ...
            res = [r.type_id.line_ids[i: j] for i, j in
                   zip([0] + idx_list, idx_list + ([size] if idx_list[-1] != size else [])) if j > 0]
            last_seq = 0
