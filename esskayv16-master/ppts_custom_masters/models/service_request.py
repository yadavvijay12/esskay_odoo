...
class ServiceRequest(models.Model):
    ...
    customer_city_id = fields.Many2one("res.city", string='City', copy=False)
    city_id = fields.Many2one("res.city", string='City', copy=False)
...
class CustomerAssetLine(models.Model):
    ...
    def _onchange_stock_lot_id(self):
        if self.stock_lot_id:
            ...
            # When you select already create record state not closed, the same configuration select same asset through the validation.
            service_request = self.env['service.request'].search(
                [('service_type_id', '=', self.ticket_id.service_type_id.id),
                 ('service_category_id', '=', self.ticket_id.service_category_id.id),
                 ('request_type_id', '=', self.ticket_id.request_type_id.id),
                 ('company_id', '=', self.env.company.id),
                 ('customer_asset_ids.stock_lot_id', '=', self.stock_lot_id.id)])
            for res in service_request:
                raise ValidationError(
                    _("This lot number (%s) is already has a ticket.",
                      self.stock_lot_id.name))

        # When you select the assets, those already created show the validation error.
        # serial_checks = self.env['parent.ticket'].search(
        #     [('stock_lot_id', '=', self.stock_lot_id.id)])
        # for res in serial_checks:
        #     raise ValidationError(
        #         _("This lot number (%s) is already has a ticket.",
        #           self.stock_lot_id.name))
