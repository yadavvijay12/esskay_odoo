...
class TasksMaster(models.Model):
    ...
    category = fields.Selection([('action', 'Python Code'), ('status', 'Status'), ('system', 'System Update')],
    ...
    is_warranty_void = fields.Boolean(string="Warranty Void")
    ...
    def write(self, vals):
        if 'python_code' in vals and self.category == 'action' and not self.env.user.has_group(
                'ppts_service_request.service_request_administrator'):
            raise UserError("You do not have permission to change python code")
        elif self.category == 'system' and 'category' in vals and not self.env.user.has_group(
                'ppts_service_request.service_request_administrator'):
            raise UserError(_("You do not have permission to change the category from System Update to %s" % dict(
                self._fields['category'].selection).get(vals.get('category'))))
        # complaint_type = dict(self._fields['category'].selection).get(vals.get('category'))
        rec = super().write(vals)
        return rec

    def unlink(self):
        if self.python_code or self.category == 'system' and not self.env.user.has_group(
                'ppts_service_request.service_request_administrator'):
