...
class APIController(http.Controller):
    ...
    def _api_get_child_ticket_value(self, **post):
        ...
        for rec in child_ticket_ids:
            ...
            create_date = {'key': 'create_date', 'type': rec._fields['create_date'].type, 'title': rec._fields['create_date'].string,
                      'value': str(rec.create_date.astimezone(local_tz).replace(tzinfo=None).strftime('%B %d,%Y %I:%M %p')) if rec.create_date else ''}
            ...
            repair_location_id = {'key': 'repair_location_id', 'type': rec._fields['repair_location_id'].type, 'title': rec._fields['repair_location_id'].string,
            ...
            extended_warranty_status ={'key': 'extended_warranty_status', 'type': rec._fields['extended_warranty_status'].type, 'title': rec._fields['extended_warranty_status'].string,
                    'value': dict(rec._fields['extended_warranty_status'].selection).get(rec.extended_warranty_status) if rec.extended_warranty_status else ''}
            ...
            requested_by_email_ct = {'key': 'requested_by_email_ct', 'type': rec._fields['requested_by_email_ct'].type, 'title': rec._fields['requested_by_email_ct'].string,
                    'value': rec.requested_by_email_ct if rec.requested_by_email_ct else ''}
            requested_by_title_ct = {'key': 'requested_by_title_ct', 'type': rec._fields['requested_by_title_ct'].type, 'title': rec._fields['requested_by_title_ct'].string,
            ...
            alternate_contact_email = {'key': 'alternate_contact_email', 'type': rec._fields['alternate_contact_email'].type, 'title': rec._fields['alternate_contact_email'].string,
                    'value': rec.alternate_contact_email if rec.alternate_contact_email else ''}
            alternate_contact_id ={ 'key': 'alternate_contact_id', 'type': rec._fields['alternate_contact_id'].type, 'title': rec._fields['alternate_contact_id'].string,
            ...
            re_repair = {'key': 're_repair', 'type': rec._fields['re_repair'].type, 'title': rec._fields['re_repair'].string,
                    'value': dict(rec._fields['re_repair'].selection).get(rec.re_repair) if rec.re_repair else ''}
            parent_ticket_id_alias = {'key': 'parent_ticket_id_alias', 'type': rec._fields['parent_ticket_id_alias'].type, 'title': rec._fields['parent_ticket_id_alias'].string,
                    'value': rec.parent_ticket_id_alias if rec.parent_ticket_id_alias else ''}
            parent_ticket_id_alias_date = {'key': 'parent_ticket_id_alias_date', 'type': rec._fields['parent_ticket_id_alias_date'].type, 'title': rec._fields['parent_ticket_id_alias_date'].string,
                      'value': str(rec.parent_ticket_id_alias_date.strftime('%B %d,%Y')) if rec.parent_ticket_id_alias_date else ''}
            action_taken_at_site = {'key': 'action_taken_at_site', 'type': rec._fields['action_taken_at_site'].type, 'title': rec._fields['action_taken_at_site'].string,
            ...
            job_closed_date = {'key': 'job_closed_date', 'type': rec._fields['job_closed_date'].type, 'title': rec._fields['job_closed_date'].string,
                      'value': str(rec.job_closed_date.astimezone(local_tz).replace(tzinfo=None).strftime('%B %d,%Y %I:%M %p')) if rec.job_closed_date else ''}
            installation_id = {'key': 'installation_id', 'type': rec._fields['installation_id'].type, 'title': rec._fields['installation_id'].string,
                    'value': rec.installation_id.name if rec.installation_id else '','ids':[rec.installation_id.id] if rec.installation_id else ''}
            replacement_order_ids = {'key':'replacement_order_ids','type':rec._fields['replacement_order_ids'].type,
                                     ...
                                     'value' : ', '.join(rec.replacement_order_ids.mapped('name')) if rec.replacement_order_ids else '',
                                     'ids':rec.replacement_order_ids.mapped('id') if rec.replacement_order_ids else False,
                                     }
            ...
            child_ticket_list.append(
                [id, name,child_ticket_type_id,customer_type,create_date,state,product_id, child_ticket_id_alias, child_configuration_id, call_source_id,
                 call_date,request_type_id,service_type_id,service_category_id, stock_lot_id,product_part_number,repair_location_id,
                 oem_warranty_status, oem_repair_status,extended_warranty_status, categ_id, problem_description,requested_by_name_child,
                 requested_by_contact_number,requested_by_email_ct,requested_by_title_ct,remarks,dealer_distributor_id,
                 call_received_id,partner_id,alternate_contact_name,alternate_contact_email,alternate_contact_id,alternate_contact_number,
                 oem_warranty_status_id,repair_warranty_status_id,cmc_status,amc_status, current_asset_location,team_id,customer_account_id,
                 parent_ticket_id,service_request_id,faulty_section,sow,webdelata_id,mc_stk,company_id,survey_id,price_available_in_contract,
                 re_repair,parent_ticket_id_alias,parent_ticket_id_alias_date,action_taken_at_site,version,job_closed_date,installation_date,
            ...
            if rec.state not in ['cancel','closed'] and not rec.is_ar_hold_tick and not rec.is_disable_status_update and attendance:
