...
class APIController(http.Controller):
    ...
    def _api_get_expenses_details(self, **post,):
        ...
        hr_expense = hr_expense_obj.sudo().search_read([('employee_id.user_id', '=', uid),('company_id','=',user.company_id.id)],fields=['name','state','product_id','total_amount','currency_id','tax_ids','company_id','employee_id','payment_mode','amount_tax'])
        for rec in hr_expense:
            expenses = hr_expense_obj.sudo().browse(rec.get('id'))
            rec['date'] = str(expenses.date)
            rec['payment_mode'] = dict(expenses._fields['payment_mode'].selection).get(expenses.payment_mode)
        if hr_expense:
