...
class ParentTicket(models.Model):
    ...
    is_status_reassign = fields.Boolean(string="Re Assign Status update")
    ...
    call_date = fields.Datetime(string="Call Received Date", default=fields.Datetime.now)
    ...
    problem_description = fields.Text(string='Problem Reported', required=True)
    ...
    state = fields.Selection(
        [('new', 'New'), ('warranty_check', 'OEM Warranty Approval'), ('paid_repair_approval', 'Paid Repair Approval'),
    ...
    is_paid_repair_approval_required = fields.Boolean(string='Paid Repair Approval', copy=False)
    ...
    ct_engineer_ids = fields.Many2many('res.users', string='Child Ticket Engineers', copy=False)
    ...
    sale_quotation_count = fields.Integer(string='Approval Count', compute='_compute_sale_quotation_count')
    ...
    def _compute_partner_dealer_check(self):
        ...
        for rec in self:
            # Search for the partner record that matches the name in the current record's partner_id
    ...
    def _onchange_requested_contact_check(self):
        # When you enter the contact number,the contact number already creates a contact person. Once you enter the same contact number, the email and title are automatically shown in these fields.
    ...
    def _onchange_requested_email_check(self):
        # When you enter the email, the email already creates a contact person. Once you enter the email, the contact name person's mobile number are automatically created in the mobile field.
    ...
    def _onchange_partner_id(self):
        # When you select the customer name, it automatically creates the customer address fields.
        stock_lot_ids = None
        if self.partner_id:
            ...
            # when you select the customer, the customer againsts create assets also closed state assets will be show.
            stock_lot_ids = self.env['stock.lot'].search(
                [('customer_id', '=', self.partner_id.id), ('state', '=', 'state_active')])
            if stock_lot_ids:
                existing_ticket_lot_ids = self.env['parent.ticket'].search(
                    [('company_id', '=', self.env.user.company_id.id), ('partner_id', '=', self.partner_id.id),
                     ('stock_lot_id', 'in', stock_lot_ids.ids), ('state', 'not in', ('closed', 'rejected'))]).mapped(
                    'stock_lot_id')
                if existing_ticket_lot_ids:
                    common_available_lots = list(set(stock_lot_ids.ids) - set(existing_ticket_lot_ids.ids))
                    domain = [('id', 'in', common_available_lots)]
                else:
    ...
    def checking_stock_lot_id(self):
        # When you select the assets, those already created show the validation error.
        if self.stock_lot_id:
            value = self.env['parent.ticket'].search(
                [('stock_lot_id', '=', self.stock_lot_id.id), ('service_type_id', '=', self.service_type_id.id),
                 ('state', '!=', 'closed')])
            ...
            for rec in self:
                if rec.amc_status == "out_warranty":
                    raise ValidationError('There is no warranty in AMC status')
                if rec.cmc_status == "out_warranty":
                    raise ValidationError('There is no warranty in CMC status')
    ...
    def action_open_customer_contacts(self):
        ...
        if partner_mobile_id:
            ...
            return True
        if not title_id:
            title_id = self.env['res.partner.title'].sudo().create({'name': self.requested_by_title})
        if title_id:
    ...
    def action_create_sale(self):
        for order in self:
            ...
            message = _("Successfully Created Sale Quotation.")
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
    ...
    def partner_amc_cmc_status_check(self):
        ...
        if self.amc_status == 'in_warranty' and self.service_category_id.category_type == 'is_amc':
            ...
            self.check_status = True
        elif self.service_category_id.category_type == 'is_warranty' and (
                self.oem_warranty_status == 'in_oem_warranty' or self.parent_configuration_id.oem_inwarranty_approval == 'yes'):
            self.check_status = True
        elif self.service_category_id.category_type == 'is_extended_warranty' and self.extended_warranty_status == 'in_repair_warranty':
            self.check_status = True
        elif self.service_category_id.category_type == 'is_repair_warranty' and (
                self.oem_repair_status == 'in_repair_warranty' or self.parent_configuration_id.repair_inwarranty_approval == 'yes'):
            self.check_status = True
    ...
    #     if self.partner_id:
    #         stock_lot_ids = self.env['stock.lot'].search([('customer_id', '=', self.partner_id.id),('id','in',self.partner_id.child_ticket_ids.filtered(lambda x: x.state == 'closed').mapped('stock_lot_id'))])
    #         # domain = [('id', 'in',stock_lot_ids.customer_id.child_ticket_ids.filtered(lambda x: x.state == 'closed').ids)]
    #         domain = [('id','in',stock_lot_ids.ids)]
    #     else:
    #         domain = [('id', 'in', None)]
    #     return {'domain': {'stock_lot_id': domain}}
    ...
    def action_confirm(self):
        ...
        self.env['asset.lot.serial'].create({
            ...
            'contract_details': self.contract_ids.ids,
            'status': rec,
        })
        if self.is_warranty_approval_required:
            self.state = 'warranty_check'
            self.warranty_send_for_approval()
        elif self.is_repair_warranty_approval_required:
            self.state = 'repair_warranty_check'
            self.warranty_send_for_approval()
        elif self.is_paid_repair_approval_required:
            self.state = 'paid_repair_approval'
            self.warranty_send_for_approval()
        else:
            self.state = 'confirm'
    ...
    def get_repair_warranty_approval_required(self):
        ...
        if self.parent_configuration_id.oem_warranty_check == 'yes':
            ...
            if self.parent_configuration_id.oem_outwarranty_approval == 'yes' and self.oem_warranty_status == 'out_oem_warranty':
                self.is_warranty_approval_required = True
        if self.parent_configuration_id.paid_repair_approved:
            self.is_paid_repair_approval_required = True
    ...
    def warranty_send_for_approval(self):
        ...
        if self.parent_configuration_id.repair_inwarranty_approval or self.parent_configuration_id.repair_outwarranty_approval:
            ...
            for repair_approval in self.parent_configuration_id.repair_approval_team_ids:
                ...
                requests.parent_ticket_id = self.id
        if self.parent_configuration_id.paid_repair_approved:
            if not self.parent_configuration_id.sudo().repair_approval_team_ids:
                raise UserError(_("There is no approval teams for Paid Repair approval"))
            for repair_approval in self.parent_configuration_id.sudo().repair_approval_team_ids:
                request = {
                    'name': "Paid Repair Approval " + self.name,
                    'type_id': repair_approval.id or False,
                    'parent_ticket_id': self.id
                }
    ...
    def write(self, vals):
        # if self.assign_engineer_ids.ids and self.env.user.id not in self.assign_engineer_ids.ids:
        #     raise UserError('You are not allowed to modify this Ticket')
    ...
    def action_create_child_ticket_wizard(self):
        open_child_tickets = self.env['child.ticket'].search(
            [('parent_ticket_id', '=', self.id), ('service_type_id', '=', self.service_type_id.id),
    ...
    # Smart Button - Quotation - Count
    def _compute_sale_quotation_count(self):
        for record in self:
            record.sale_quotation_count = self.env['sale.order'].search_count([('parent_ticket_id', '=', record.id)])
...
class TasksMasterLine(models.Model):
    ...
    approval_type_id = fields.Many2one(string="Approval Type", comodel_name="multi.approval.type")
    ...
    def onchange_attachment_ids(self):
        for rec in self:
            if rec.attachment_ids:
                rec.is_check_attachment = True

    def check_warranty_void(self):
        if self.child_ticket_id and self.task_id.is_warranty_void:
            self.child_ticket_id.stock_lot_id.custom_asset_warranty_line.is_active = False
        if self.parent_ticket_id and self.task_id.is_warranty_void:
            self.parent_ticket_id.stock_lot_id.custom_asset_warranty_line.is_active = False

    def action_request_approval(self):
        for rec in self:
            if rec.task_id.is_request_approval and rec.parent_ticket_id:
                if not rec.approval_type_id:
                    raise ValidationError(
                        _('Required Approval Type for this Task : %s', rec.task_id.name))
                request = {
                    ...
                    'team_id': rec.parent_ticket_id.team_id.id
                }
                requests = self.env['multi.approval'].sudo().create(request)
                requests.action_submit()
                ...
                request = {
                    ...
                    'team_id': rec.child_ticket_id.team_id.id
                }
                requests = self.env['multi.approval'].sudo().create(request)
                requests.action_submit()
    ...
    def onchange_action_taken_check(self):
        if self.action_taken:
            child = self.env['child.ticket'].browse(self.child_ticket_id._origin.id)
            child.action_taken_at_site = self.action_taken

    @api.onchange('task_id')
    def onchange_task_list(self):
        if self.task_id.is_request_approval and self.task_id.approval_type_id:
            self.approval_type_id = self.task_id.approval_type_id.id
        ...
        if (self.task_id.is_pi_required or self.task_id.name == "PI Not Required") and not self.env.user.has_group(
        ...
        # if self.child_ticket_id and self.task_id.is_work_end:
        #     self.action_taken = self.child_ticket_id.action_taken_at_site

        if self.child_ticket_id and self.task_id.is_pi_required:
            self.date_of_event = self.child_ticket_id.service_request_id.date_of_event
            self.awareness_date = self.child_ticket_id.service_request_id.awareness_date
            self.event_description = self.child_ticket_id.service_request_id.event_description
    ...
    def button_tasks_update(self):
        for record in self:
            ...
            if record.task_id.name == 'Start Travel for Equipment Handover' and record.child_ticket_id:
                    ...
                    if picking.picking_type_id.code == 'outgoing' and picking.state not in ('done', 'cancel'):
                        raise ValidationError(
                            _("Confirm this delivery order (%s) to proceed with this status (%s).", picking.name,
                              record.task_id.name))
            # if record.is_notify:
            #     if record.child_ticket_id.child_assign_engineer_ids:
            #         assign_engineer_email = record.child_ticket_id.child_assign_engineer_ids[-1].login
            #         template_id = self.env.ref('ppts_parent_ticket.notify_email_template')
            #         if template_id:
            #             template_id.with_context(email_to=assign_engineer_email, email_cc=mails_cc).sudo().send_mail(
            #                 record.child_ticket_id.id,
            #                 force_send=True)

            # Check if the tasks is work order to check whether the stock is valiated or not
            if record.task_id.is_work_order:
                for stock_record in self.env['stock.picking'].sudo().search(
                        [('child_ticket_id', '=', record.child_ticket_id.id)]):
                    if stock_record.picking_type_id.code in ('incoming', 'internal') and stock_record.state not in (
                            'done', 'cancel'):
                        raise UserError(
                            _("Validate the stock before creating work order. Click on Smart button to open the receipts!"))
            ...
            if record.parent_ticket_id and record.task_id.is_ressign_engineer:
                record.parent_ticket_id.is_request_reassign = record.task_id.is_ressign_engineer
            if record.child_ticket_id and record.task_id.is_ressign_engineer:
            ...
            # Trigger File Attachments function
            if record.is_file_attachment and (record.parent_ticket_id or record.child_ticket_id):
                record.action_store_attachments()
            if record.task_id.is_warranty_void:
                record.check_warranty_void()

    def action_store_attachments(self):
        if self.attachment_ids and self.sudo().parent_ticket_id:
            self.attachment_ids.public = True
            self.sudo().parent_ticket_id.attachment_ids = self.attachment_ids.ids
        elif self.attachment_ids and self.child_ticket_id:
            self.attachment_ids.public = True
            self.child_ticket_id.child_ticket_attachments = self.attachment_ids.ids

    # work flow parent ticket create status (boolean).
    def action_parent_ticket_create(self):
        # self.ensure_one()
        # When u add status parent ticket to parent ticket create
        parent_ticket = self.sudo().parent_ticket_id
        if parent_ticket:
            data = {
                'product_id': parent_ticket.product_id.id or False,
                'partner_id': parent_ticket.partner_id.id or False,
                'call_source_id': parent_ticket.call_source_id.id or False,
                'product_category_id_alias': parent_ticket.product_category_id_alias or '',
                'requested_by_name': parent_ticket.requested_by_name or '',
                'requested_by_contact_number': parent_ticket.requested_by_contact_number or '',
                'requested_by_email': parent_ticket.requested_by_email or '',
                'requested_by_title': parent_ticket.requested_by_title or '',
                'call_received_id': parent_ticket.call_received_id.id or False,
                'oem_warranty_status_id': parent_ticket.oem_warranty_status_id.id or False,
                'repair_warranty_status_id': parent_ticket.repair_warranty_status_id.id or False,
                'team_id': parent_ticket.team_id.id or False,
                'state': 'new',
                'remarks': parent_ticket.remarks or '',
                'problem_description': parent_ticket.problem_description or '',
                'description': parent_ticket.description or '',
                'dealer_distributor_id': parent_ticket.dealer_distributor_id.id or False,
                'stock_lot_id': parent_ticket.stock_lot_id.id or False,
                'categ_id': parent_ticket.categ_id.id or False,
                'parent_ticket_id_alias': parent_ticket.parent_ticket_id_alias or '',
                ...
                'parent_ticket_id_alias_date': parent_ticket.parent_ticket_id_alias_date,
                'cat_no': parent_ticket.cat_no,
                'product_code_no': parent_ticket.product_code_no,
                'external_work_order_date': parent_ticket.external_work_order_date,
                'price_available_in_contract': parent_ticket.price_available_in_contract,
                'alternate_contact_email': parent_ticket.alternate_contact_email,
                'alternate_contact_name': parent_ticket.alternate_contact_name,
                'alternate_contact_number': parent_ticket.alternate_contact_number,
                'faulty_section': parent_ticket.faulty_section,
                're_repair': parent_ticket.re_repair,
                'exp_no': parent_ticket.exp_no,
                'pi_no': parent_ticket.pi_no,

                # 'packaging_id': parent_ticket.packaging_id,
                # 'packaging_alias': parent_ticket.packaging_alias,
            ...
            # When u add status child ticket to parent ticket create
            parent_ticket_ct = None
            data = {
                ...
                'parent_ticket_id_alias_date': self.child_ticket_id.parent_ticket_id_alias_date,
                'cat_no': self.child_ticket_id.product_part_number,
                'product_code_no': self.child_ticket_id.product_id.product_code,
                'price_available_in_contract': self.child_ticket_id.price_available_in_contract,
                'alternate_contact_name': self.child_ticket_id.alternate_contact_name,
                'alternate_contact_number': self.child_ticket_id.alternate_contact_number,
                'alternate_contact_email': self.child_ticket_id.alternate_contact_email,
                'requested_by_email': self.child_ticket_id.requested_by_email_ct,
                'requested_by_title': self.child_ticket_id.requested_by_title_ct,
                're_repair': self.child_ticket_id.re_repair,
                'faulty_section': self.child_ticket_id.faulty_section,
                'sow': self.child_ticket_id.sow,
                'webdelata_id': self.child_ticket_id.webdelata_id,
                'webdelata': self.child_ticket_id.webdelata,
                'mc_stk': self.child_ticket_id.mc_stk,

            }
            parent_ticket_ct = self.env['parent.ticket'].sudo().create(data)
            # If the request type is not need for an approval then this field must be true
            if not self.task_id.request_type_id.is_required_approval:
                parent_ticket_ct.is_send_for_approvals = True
            if not self.task_id.parent_configuration_id.work_flow_id.task_list_ids:
                raise UserError(
                    'There is no workflow configured for this type. Map the workflow configuration on the parent ticket configuration.')
            if self.task_id.parent_configuration_id.work_flow_id:
                status_ct = self.task_id.parent_configuration_id.work_flow_id.task_list_ids[0]
                status_data_ct = [(0, 0, {
                    'task_id': status_ct.task_id.id,
                    'next_task_ids': [(6, 0, status_ct.next_task_ids.ids)],
                    'status': status_ct.status,
                    'parent_ticket_id': parent_ticket_ct.id,
                })]
                parent_ticket_ct.write({'task_list_ids': status_data_ct})
            parent_ticket_ct._onchange_stock_lot_id()
            parent_ticket_ct._onchange_task_list_ids()
            parent_ticket_ct.onchange_service()
            parent_ticket_ct._onchange_request_type_id()
            parent_ticket_ct._onchange_request_type()
            parent_ticket_ct.get_repair_warranty_approval_required()
            parent_ticket_ct.partner_amc_cmc_status_check()

    # create new service request (enable boolean)
    def create_sr(self):
        self.ensure_one()
        ...
        if self.parent_ticket_id:
            ...
            if existing_service_request_id:
                ...
                # new_service_request_id.customer_address_check()

            else:
                existing_service_request_id = {
                    'request_type_id': self.task_id.request_type_id.id,
                    'service_category_id': self.task_id.service_category_id.id,
                    'service_type_id': self.task_id.service_type_id.id,
                    'partner_id': self.parent_ticket_id.partner_id.id,
                    'service_request_date': self.parent_ticket_id.parent_ticket_id_alias_date,
                    'parent_ticket_id': self.parent_ticket_id.id,
                    'service_request_id_alias': self.parent_ticket_id.parent_ticket_id_alias,
                    'call_source_id': self.parent_ticket_id.call_source_id.id,
                    'problem_reported': self.parent_ticket_id.problem_description,
                    'requested_by_name': self.parent_ticket_id.requested_by_name,
                    'requested_by_contact_number': self.parent_ticket_id.requested_by_contact_number,
                    'requested_by_email': self.parent_ticket_id.requested_by_email,
                    'requested_by_title': self.parent_ticket_id.requested_by_title,
                    'customer_name': self.parent_ticket_id.partner_id.name,
                    'custom_product_serial': self.parent_ticket_id.stock_lot_id.id,
                    'product_name': self.parent_ticket_id.product_id.name,
                    'product_part_number': self.parent_ticket_id.cat_no,
                    'product_part_code': self.parent_ticket_id.product_code_no,
                    'call_received_id': self.parent_ticket_id.call_received_id.id,
                    'remarks': self.parent_ticket_id.remarks,
                    'attachment_ids': self.parent_ticket_id.attachment_ids,
                    'reason': self.parent_ticket_id.problem_description,
                    ...
                    'alternate_contact_email': self.parent_ticket_id.alternate_contact_email,
                    'customer_asset_ids': [(0, 0, {
                        'stock_lot_id': self.parent_ticket_id.stock_lot_id.id,
                        'product_id': self.parent_ticket_id.product_id.id,
                    })],
                }
                service_request = self.env['service.request'].sudo().create(existing_service_request_id)
                service_request._onchange_partner()
                # service_request.customer_address_check()
            ...
            if existing_service_request_id:
                ...
                new_service_request_id.service_request_date = fields.Datetime.today()
                ...
                new_service_request_id.alternate_contact_name = self.child_ticket_id.alternate_contact_name or False
                new_service_request_id.alternate_contact_number = self.child_ticket_id.alternate_contact_number or False
                new_service_request_id.alternate_contact_email = self.child_ticket_id.alternate_contact_email or False
                new_service_request_id._onchange_partner()
                new_service_request_id.customer_address_check()
                stock_lines = []
                ...
                existing_service_request_id = {
                    ...
                    'service_category_id': self.task_id.service_category_id.id,
                    'service_type_id': self.task_id.service_type_id.id,
                    'partner_id': self.child_ticket_id.partner_id.id,
                    'service_request_date': self.child_ticket_id.parent_ticket_id_alias_date if self.child_ticket_id.parent_ticket_id_alias_date else fields.Datetime.today(),
                    'child_ticket_id': self.child_ticket_id.id,
                    'service_request_id_alias': self.child_ticket_id.child_ticket_id_alias,
                    'call_source_id': self.child_ticket_id.call_source_id.id,
                    'problem_reported': self.child_ticket_id.problem_description,
                    'requested_by_name': self.child_ticket_id.requested_by_name_child,
                    'requested_by_contact_number': self.child_ticket_id.requested_by_contact_number,
                    'requested_by_email': self.child_ticket_id.requested_by_email_ct,
                    'requested_by_title': self.child_ticket_id.requested_by_title_ct,
                    'customer_name': self.child_ticket_id.partner_id.name,
                    ...
                    'call_received_id': self.child_ticket_id.call_received_id.id,
                    'reason': self.child_ticket_id.problem_description,
                    'remarks': self.child_ticket_id.remarks,
                    'alternate_contact_name': self.child_ticket_id.alternate_contact_name,
                    'alternate_contact_number': self.child_ticket_id.alternate_contact_number,
                    'alternate_contact_email': self.child_ticket_id.alternate_contact_email,
                    'customer_email': self.child_ticket_id.partner_id.email,
                    'product_part_code': self.child_ticket_id.product_code_no,
                    'description': self.child_ticket_id.description,

                    # 'oem_warranty_status_id': self.child_ticket_id.oem_warranty_status.id or False,
                    # 'oem_repair_status': self.child_ticket_id.repair_warranty_status_id.id,
                    # 'attachment_ids':self.child_ticket_id.attachment_ids,
                    'customer_asset_ids': [(0, 0, {
                        'stock_lot_id': self.child_ticket_id.stock_lot_id.id,
                        'product_id': self.child_ticket_id.product_id.id,
                    })],
    ...
    def mail_ct_new_ticket_creation(self, child_ticket_id):
        template_id = self.env.ref('ppts_parent_ticket.new_child_ticket_creation_template')
        ct_user_to_address = child_ticket_id.parent_ticket_id.assign_engineer_ids.mapped('email')
        if template_id and ct_user_to_address:
            mail_id = template_id.with_context(email_to=','.join(ct_user_to_address)).sudo().send_mail(
                child_ticket_id.id, force_send=True)
        return True
    ...
    def create_child_ticket(self):
        ...
        if self.parent_ticket_id:
            data = {
                ...
                'product_part_number': self.parent_ticket_id.cat_no,
                'requested_by_email_ct': self.parent_ticket_id.requested_by_email,
                'requested_by_title_ct': self.parent_ticket_id.requested_by_title,
                'amc_status': self.parent_ticket_id.amc_status,
                'cmc_status': self.parent_ticket_id.cmc_status,
                # 'survey_id':self.parent_ticket_id.survey_id,
                'price_available_in_contract': self.parent_ticket_id.price_available_in_contract,
                're_repair': self.parent_ticket_id.re_repair,
                'parent_ticket_id_alias': self.parent_ticket_id.parent_ticket_id_alias,
            ...
            user_id = self.child_ticket_id.parent_ticket_id.assign_engineer_ids[-1] if self.child_ticket_id else \
                self.sudo().parent_ticket_id.assign_engineer_ids[-1]
            model_id = self.env['ir.model'].sudo().search([('model', '=', 'parent.ticket')])
            self.env['mail.activity'].sudo().create(
                {'activity_type_id': 4, 'user_id': user_id.id,
                 'summary': child_ticket.service_type_id.name + ' is created and the reference number is' + ' ' + str(
                     child_ticket.name) + ' and the parent reference number is ' + str(
                     self.child_ticket_id.sudo().parent_ticket_id.name or self.sudo().parent_ticket_id.name),
            ...
            data = {
                ...
                'categ_id': self.child_ticket_id.categ_id.id or False,
                'product_category_id_alias': self.child_ticket_id.product_category_id_alias or '',
                'problem_description': BeautifulSoup(self.Commants, "lxml").text if self.Commants else '',
                ...
                'description': self.child_ticket_id.description,
                # 'survey_id': self.child_ticket_id.survey_id,
                'price_available_in_contract': self.child_ticket_id.price_available_in_contract,
                're_repair': self.child_ticket_id.re_repair,
                'parent_ticket_id_alias': self.child_ticket_id.parent_ticket_id_alias,

            }
            child_ticket = self.env['child.ticket'].sudo().create(data)
            # Send mail to CT since the ticket is created automatically
            self.mail_ct_new_ticket_creation(child_ticket)
            # if request type is an approval then this field must be true to send for an approval
            if not self.task_id.request_type_id.is_required_approval:
                child_ticket.is_send_for_approvals = True
            if self.task_id.child_configuration_id.work_flow_id:
                ...
                child_ticket.sudo().write({'task_list_ids': status_data})
                # Create notification for the CT user.
                user_id = self.child_ticket_id.sudo().parent_ticket_id.assign_engineer_ids[-1]
                model_id = self.env['ir.model'].sudo().search([('model', '=', 'parent.ticket')])
                self.env['mail.activity'].sudo().create(
                    {'activity_type_id': 4, 'user_id': user_id.id,
                     'summary': child_ticket.service_type_id.name + 'is created and the reference number is' + ' ' + child_ticket.name + ' and the parent reference number is ' + self.child_ticket_id.sudo().parent_ticket_id.name,
                     'res_model': 'parent.ticket', 'res_model_id': model_id.id,
...
class MultiApproval(models.Model):
    ...
    partner_id = fields.Many2one('res.partner', string='Customer Name', compute='_get_customer_details')
    ...
    engineer_comments = fields.Char(string='Engineer Comments(At Job Closed)')
    team_id = fields.Many2one('crm.team', string="Team", help="Should inherit from user & Edit")
    line_comments = fields.Html('Engineer Comments')

    customer_problem_description = fields.Text(string='Customer Problem Description', copy=False)
    oem_warranty_status = fields.Selection(
        [('in_oem_warranty', 'In OEM Warranty'), ('out_oem_warranty', 'Out Of OEM Warranty'),
         ('not_available', 'Not available')], string='OEM Warranty Status')
    oem_repair_status = fields.Selection(
    ...
    @api.depends('parent_ticket_id', 'child_ticket_id', 'service_request_id')
    def _get_customer_details(self):
        for record in self:
            if record.parent_ticket_id:
                record.partner_id = record.parent_ticket_id.partner_id.id
                record.customer_type = record.partner_id.customer_type.id
                record.cmc_status = record.parent_ticket_id.cmc_status
                record.amc_status = record.parent_ticket_id.amc_status
                record.customer_problem_description = record.parent_ticket_id.problem_description
                record.oem_warranty_status = record.parent_ticket_id.oem_warranty_status
                record.oem_repair_status = record.parent_ticket_id.oem_repair_status
                record.extended_warranty_status = record.parent_ticket_id.extended_warranty_status
                record.repair_location_id = record.parent_ticket_id.repair_center_location_id.id
                # record.attachment_ids = record.parent_ticket_id.attachment_ids
            elif record.child_ticket_id:
                record.partner_id = record.child_ticket_id.partner_id.id
                record.customer_type = record.partner_id.customer_type.id
                record.cmc_status = record.child_ticket_id.cmc_status
                record.amc_status = record.child_ticket_id.amc_status
                record.action_taken_at_site = record.child_ticket_id.action_taken_at_site
                record.customer_problem_description = record.child_ticket_id.problem_description
                record.oem_warranty_status = record.child_ticket_id.oem_warranty_status
                record.oem_repair_status = record.child_ticket_id.oem_repair_status
                record.extended_warranty_status = record.child_ticket_id.extended_warranty_status
                record.repair_location_id = record.child_ticket_id.repair_location_id.id

                # record.attachment_ids = record.child_ticket_id.child_ticket_attachments
                task_status = record.child_ticket_id.task_list_ids.filtered(lambda l: l.task_id.name == 'Job Closed')
                approval_status_id = record.child_ticket_id.task_list_ids.filtered(
                    lambda l: l.task_id.is_request_approval == True)
                if task_status:
                    record.engineer_comments = task_status.recommendation_customer
                if approval_status_id:
                    # pass
                    record.line_comments = approval_status_id.Commants
            elif record.service_request_id:
                record.partner_id = record.service_request_id.partner_id.id
                record.customer_type = record.service_request_id.customer_type_id.id
                # record.attachment_ids = record.service_request_id.attachment_ids
            else:
                record.partner_id = False
                record.customer_type = False
                record.cmc_status = False
                record.amc_status = False
                record.action_taken_at_site = False
                record.customer_problem_description = False
                record.oem_warranty_status = False
                record.oem_repair_status = False
                record.extended_warranty_status = False
    ...
    def action_approve(self):
        ...
        if self.parent_ticket_id.id and self.parent_task_line_id:
            if self.state == 'Approved' and not self.parent_task_line_id.is_request_approval:
                ...
                approved_task = self.parent_ticket_id.parent_configuration_id.work_flow_id.task_list_ids.task_id.filtered(
                    lambda r: r.name == 'Approved')
                if approved_task:
                    self.parent_ticket_id.sudo().write(
                ...
                self.env['mail.message'].create(values)
                self.parent_ticket_id.sudo().message_post(body='As per the request the status has been approved.')
        elif self.parent_ticket_id.id:
            if self.state != 'Approved':
                pass
            elif (
                    self.parent_ticket_id.is_repair_warranty_approval_required or self.parent_ticket_id.is_warranty_approval_required or self.parent_ticket_id.is_paid_repair_approval_required) and not self.parent_ticket_id.is_warranty_approval_done:
            ...
            #     parent_ticket.sudo().write({'state': 'confirm', 'is_warranty_approval_done': True})
            # elif self.parent_ticket_id.is_paid_repair_approval_required and not self.parent_ticket_id.is_warranty_approval_done and self.state == 'Approved':
            #     parent_ticket.sudo().write({'state': 'confirm', 'is_warranty_approval_done': True})
            else:
