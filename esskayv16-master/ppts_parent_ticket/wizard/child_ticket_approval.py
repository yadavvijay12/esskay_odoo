...
class ChildTicketApproval(models.Model):
    ...
    def action_send_for_approval(self):
        ...
        if child_ticket_id and 'child_task_line_id' in self.env.context:
            ...
            attachments = []
            for record in self:
                if child_ticket_id.child_ticket_attachments:
                    for attachment in child_ticket_id.child_ticket_attachments:
                        ir_values = {
                ...
                request = {
                    ...
                    'type_id': record.type_id.id or False,
                    'child_ticket_id': child_ticket_id.id,
                    'team_id': child_ticket_id.team_id.id or False,
                ...
                warranty = dict(child_ticket_id._fields['oem_warranty_status'].selection).get(
                    child_ticket_id.oem_warranty_status)
                repair = dict(child_ticket_id._fields['oem_repair_status'].selection).get(
                    child_ticket_id.oem_repair_status)
                extended = dict(child_ticket_id._fields['extended_warranty_status'].selection).get(
                    child_ticket_id.extended_warranty_status)
                mail = ''
                for lines in requests.line_ids:
                    mail = lines.user_id.mapped('login')
                    break
                if template_id:
                    template_id.with_context(rec_url=base_url, email_to=mail, amc=amc, cmc=cmc, warranty=warranty,
                                             repair=repair, extended=extended).sudo().send_mail(requests.id,
                                                                                                force_send=True)
