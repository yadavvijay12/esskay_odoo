...
class ParentTicketApproval(models.Model):
    ...
    def action_send_for_approval(self):
        ...
        if parent_ticket_id and 'parent_task_line_id' in self.env.context:
            ...
            attachments = []
            for record in self:
                if parent_ticket_id.attachment_ids:
                    for attachment in parent_ticket_id.attachment_ids:
                        ir_values = {
                ...
                request = {
                    ...
                    'type_id': record.type_id.id or False,
                    'team_id': parent_ticket_id.team_id.id or False,
                ...
                warranty = dict(parent_ticket_id._fields['oem_warranty_status'].selection).get(
                    parent_ticket_id.oem_warranty_status)
                repair = dict(parent_ticket_id._fields['oem_repair_status'].selection).get(
                    parent_ticket_id.oem_repair_status)
                extended = dict(parent_ticket_id._fields['extended_warranty_status'].selection).get(
                    parent_ticket_id.extended_warranty_status)
