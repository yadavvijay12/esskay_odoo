    ...
    def compute_replacement_order_count(self):
        for record in self:
            record.replacement_order_count = self.env['sale.order'].search_count(
                [('is_replacement_order', '=', True), ('parent_ticket_id', '=', self.id)])
            replacement_picking = self.env['sale.order'].search(
                [('is_replacement_order', '=', True), ('parent_ticket_id', '=', self.id)]).delivery_number
...
class ChildTicketInherit(models.Model):
    ...
    def compute_replacement_order_count(self):
        for record in self:
            record.replacement_order_count = self.env['sale.order'].search_count(
                [('is_replacement_order', '=', True), ('child_ticket_id', '=', self.id),
                 ('wr_state', '!=', 'cancelled')])
            replacement_picking = self.env['sale.order'].search(
                [('is_replacement_order', '=', True), ('child_ticket_id', '=', self.id)]).delivery_number
            record.sudo().replacement_picking_ids = replacement_picking.ids
    ...
    def action_view_replacement_order(self):
        ...
        return {
            ...
            'res_model': 'sale.order',
            'target': 'current',
            'domain': [('is_replacement_order', '=', True), ('child_ticket_id', '=', self.id)],
        }
