...
class ServiceRequestWebsiteForm(http.Controller):
    ...
    def Strykerrequest(self, **kw):
        ...
        customer_city_ids = request.env['customer.city'].sudo().search([('active', '=', True)])
    ...
    @http.route(['/create/strykerrequest'], type='http', auth="public", methods=['POST'], website=True)
    def CreateStrykerRequest(self, **kw):
        ...
        if ticket_type == 'stryker_fsm':
            ...
            team_id = request.env['crm.team'].sudo().search([('customer_type', '=', 'stryker')], limit=1)
        ...
        elif ticket_type == 'stryker_installation':
    ...
    @http.route(['/create/thermofisherrequest'], type='http', auth="public", methods=['POST'], website=True)
    def CreateThermofisherRequest(self, **kw):
        ticket_type = kw.get('ticket_type')
        ...
        if ticket_type == 'thermofisher_fsm':
            action_id = request.env.ref('ppts_service_request.action_service_request_sr_fsm')
        elif ticket_type == 'thermofisher_installation':
            action_id = request.env.ref('ppts_service_request.action_service_request_sr_installation')
        else:
            action_id = request.env.ref('ppts_service_request.action_service_request')
