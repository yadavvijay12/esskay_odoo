...
class ContractLine(models.Model):
    ...
    product_tmpl_id = fields.Many2one('product.template', 'Product', related='product_id.product_tmpl_id',
                                      store=True, index=True)
    order_type = fields.Selection(
        [('amc_contract', 'AMC Contract'),('cmc_contract', 'CMC Contract'), ('extended_warranty', 'Extended Warranty'), ('repair_warranty', 'Repair Warranty')],
        string='Contract Type', compute='_compute_order_type')
    ...
    def onchange_service_product_id_domain(self):
        if self.service_product_id or self.product_id or self.stock_lot_id or self.contract_id.partner_id:
            product_ids = self.service_product_id.contract_product_temp_ids.mapped('id')
            product_varient_ids = self.service_product_id.contract_product_temp_ids.mapped('product_variant_id')
            domain = [('id', 'in', product_ids), ('sale_ok', '=', True)]
            domain2 = [('id', 'in', product_varient_ids.ids), ('sale_ok', '=', True)]
            domain3 = [('customer_id', '=', self.contract_id.partner_id.id)]
            return {'domain': {'product_template_id': domain, 'product_id': domain2, 'stock_lot_id': domain3}}
    ...
    def _compute_order_type(self):
        for record in self:
            record.order_type = record.service_product_id.order_type if record.service_product_id and record.service_product_id.order_type in ('amc_contract', 'cmc_contract') else ''
