...
class Survey(http.Controller):
    ...
    # ACCESS
    # ------------------------------------------------------------
    def _fetch_from_access_token(self, survey_token, answer_token, ticket_id=None, ticket_type=None):
        ...
        if not answer_token and ticket_id and ticket_type:
            if ticket_type == 'parent':
                # parent_ticket_id = request.env['parent.ticket'].sudo().browse(ticket_id)
                answer_sudo = request.env['survey.user_input'].sudo().search([('survey_id', '=', survey_sudo.id),('parent_ticket_id', '=', int(ticket_id))], limit=1)
            if ticket_type == 'child':
                # child_ticket_id = request.env['child.ticket'].sudo().browse(ticket_id)
                answer_sudo = request.env['survey.user_input'].sudo().search([('survey_id', '=', survey_sudo.id),('child_ticket_id', '=', int(ticket_id))], limit=1)
        if not answer_sudo:
            answer_sudo = request.env['survey.user_input'].sudo()
        return survey_sudo, answer_sudo

    def _check_validity(self, survey_token, answer_token, ensure_token=True, check_partner=True, ticket_id=None, ticket_type=None):
        ...
        """
        survey_sudo, answer_sudo = self._fetch_from_access_token(survey_token, answer_token, ticket_id, ticket_type)
    ...
    def _get_access_data(self, survey_token, answer_token, ensure_token=True, check_partner=True, ticket_id=None, ticket_type=None):
        ...
        validity_code = self._check_validity(survey_token, answer_token, ensure_token=ensure_token, check_partner=check_partner, ticket_id=ticket_id, ticket_type=ticket_type)
    ...
    @http.route('/survey/start/<string:survey_token>/<string:ticket_id>/<string:ticket_type>', type='http', auth='public', website=True)
    def survey_start(self, survey_token, ticket_id, ticket_type, answer_token=None, email=False, **post):
        # print(ticket_id, "ticket_id2")
        ...
        access_data = self._get_access_data(survey_token, answer_token, ensure_token=False, ticket_id=ticket_id, ticket_type=ticket_type)
        ...
        if answer_from_cookie and access_data['validity_code'] in ('answer_wrong_user', 'token_wrong'):
            ...
            access_data = self._get_access_data(survey_token, None, ensure_token=False, ticket_id=ticket_id, ticket_type=ticket_type)
        ...
        return request.redirect('/survey/%s/%s/%s/%s' % (survey_sudo.access_token, answer_sudo.access_token, ticket_id, ticket_type))
    ...
    @http.route('/survey/<string:survey_token>/<string:answer_token>/<string:ticket_id>/<string:ticket_type>', type='http', auth='public', website=True)
    def survey_display_page(self, survey_token, answer_token, ticket_id, ticket_type, **post):
        access_data = self._get_access_data(survey_token, answer_token, ensure_token=True)
        if access_data['validity_code'] is not True:
            return self._redirect_with_error(access_data, access_data['validity_code'])
    ...
    @http.route('/survey/begin/<string:survey_token>/<string:answer_token>/<string:ticket_id>/<string:ticket_type>', type='json', auth='public', website=True)
    def survey_begin(self, survey_token, answer_token, ticket_id, ticket_type, **post):
    ...
    @http.route('/survey/next_question/<string:survey_token>/<string:answer_token>/<string:ticket_id>/<string:ticket_type>', type='json', auth='public', website=True)
    def survey_next_question(self, survey_token, answer_token, ticket_id, ticket_type, **post):
    ...
    def survey_submit(self, survey_token, answer_token, **post):
        ...
        if answer_sudo.survey_time_limit_reached or survey_sudo.questions_layout == 'one_page':
            ...
            self._create_serivce_request(answer_sudo)
            if answer_sudo.state != 'new':
                outbound_calls = request.env['outbound.call.list'].sudo().search([('survey_user_input_id','=',answer_sudo.id)])
                if outbound_calls:
                    outbound_calls.unlink()
    ...
    def _create_serivce_request(self, answer_sudo):
        template_id = request.env.ref('stryker_survey.rsm_survey_escalation')
        ticket_id = False
        # survey_fail = request.env.ref("stryker_survey.view_survey_fail")
        child_success_survey = request.env.ref('stryker_survey.child_success_survey_generation')
        # survey_pass = request.env.ref("stryker_survey.view_survey_pass")
        parent_success_survey = request.env.ref('stryker_survey.parent_success_survey_generation')
        if answer_sudo.parent_ticket_id:
            ticket_id = answer_sudo.parent_ticket_id
        if answer_sudo.child_ticket_id:
            ticket_id = answer_sudo.child_ticket_id
        request_type = False
        if ticket_id:
            survey_config = ticket_id.partner_id.task_list_ids.filtered(lambda l: l.condition_id.is_create_service_request == True and l.trigger_action == 'create_sr')
            if len(survey_config) == 1:
                request_type = survey_config.request_type
            if not request_type:
                request_type = request.env['request.type'].sudo().search([('ticket_type', '=', 'sr_survey_escalation')], limit=1)
            
            service_req = False
            if survey_config and request_type:
                if answer_sudo.scoring_success == False and answer_sudo.escalated_rsm == False and answer_sudo.state == 'done':
                    service_req = request.env['service.request'].sudo().create({
                        'customer_name': ticket_id.partner_id.name if ticket_id.partner_id else False,
                        ...
                        'requested_by_contact_number': ticket_id.requested_by_contact_number if ticket_id.requested_by_contact_number else None,
                        'ticket_ref': ticket_id.name if ticket_id else False,
                        'parent_ticket_id': answer_sudo.parent_ticket_id.id if answer_sudo.parent_ticket_id else False,
                        'child_ticket_id': answer_sudo.child_ticket_id.id if answer_sudo.child_ticket_id else False,
                        'assign_engineer_ids': ticket_id.assign_engineer_ids.ids if ticket_id.assign_engineer_ids else False,
                        'is_escalation': True,
                ...
                if service_req:
                    service_req.sudo()._onchange_partner()
                    ticket_id.write({'service_request_id': service_req.id})
                    answer_sudo.sudo().write({'service_request_id': service_req.id,
                                            'escalated_rsm': True})
                ...
                if answer_sudo.scoring_success == True and answer_sudo.state == 'done':
                    survey_pass_conf = ticket_id.partner_id.task_list_ids.filtered(lambda l: l.condition_id.is_survey_passed == True and l.trigger_action == 'trigger_survey' and l.unit == 'immediate')
                    if survey_pass_conf:
                        for conf in survey_pass_conf:
                            ticket_type = 'parent' if answer_sudo.parent_ticket_id else 'child'
                            if ticket_type == 'parent':
                                url = str(conf.survey_id.survey_start_url)+'/'+str(ticket_id.id)+'/'+'parent'
                                parent_success_survey.with_context({'url': url}).sudo().send_mail(ticket_id.id, force_send=True)
                                conf.survey_id._create_answer(partner=ticket_id.partner_id, ticket_id=ticket_id.id, ticket_type=ticket_type,template_id=parent_success_survey.id)

                            if ticket_type == 'child':
                                url = str(conf.survey_id.survey_start_url)+'/'+str(ticket_id.id)+'/'+'child'
                                child_success_survey.with_context({'url': url}).sudo().send_mail(ticket_id.id, force_send=True)
                                conf.survey_id._create_answer(partner=ticket_id.partner_id, ticket_id=ticket_id.id, ticket_type=ticket_type,template_id=child_success_survey.id)
                            
                            # ticket_id.sudo().write({'url_two': str(survey_pass_conf.survey_id.survey_start_url)+'/'+str(ticket_id.id)+'/'+ticket_type})
