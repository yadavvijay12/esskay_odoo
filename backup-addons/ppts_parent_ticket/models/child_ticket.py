...
class ChildTicket(models.Model):
    ...
    dealer_distributor_id = fields.Many2one('res.partner', string="Dealer / Distributor Name", copy=False,
                                            compute='_compute_partner_dealer_check')
    ...
    team_id = fields.Many2one('crm.team', string="Team", compute='_compute_team_id', store=True, required=True,
                              readonly=True, precompute=True, copy=False, change_default=True, check_company=True,
                              tracking=True,
                              )
    is_assign_to_user = fields.Boolean(string="Is Assign to User", copy=False)
    is_request_reassign = fields.Boolean(string="Is Request Reassign", copy=False)
    ...
    is_pi_true = fields.Boolean(string="PI Required True")
    ...
    def _compute_stock_lot_id_check(self):
        for rec in self:
            ...
            if location_check:
                ...
                rec.current_asset_location = False
    ...
    def _compute_partner_dealer_check(self):
        for rec in self:
            ...
            if partner_check:
                ...
                print(rec.dealer_distributor_id.id)
            else:
    ...
    def action_view_parent_ticket_ct(self):
        ...
        return {
            ...
            'domain': [('id', '=', self.parent_ticket_id.id)],
            'context': "{'create': False}"
        }
    ...
    def write(self, vals):
        ...
        if vals.get('state') == 'closed':
            ...
            data = {
                ...
                'service_request_id': self.service_request_id.id or self.parent_ticket_id.service_request_id.id,
            ...
            open_tickets = self.env['child.ticket'].sudo().search(
                [('parent_ticket_id', '=', self.parent_ticket_id.id), ('state', '=', 'closed')])
    ...
    def get_request_ticket_type(self):
        ...
        self.request_type = self.request_type_id.ticket_type
    ...
    def _count_total_installation(self):
        for record in self:
            record.installation_count = self.env['project.task'].sudo().search_count(
                [('installation_child_ticket_id', '=', self.id)])
    ...
    def action_open_product_template(self):
        print("T", self.product_id.product_tmpl_id.id)
        self.ensure_one()
        return {
            'name': 'Product',
            'type': 'ir.actions.act_window',
        ...
        for record in self:
            ...
            if record.is_end_task and record.child_ticket_id:
                if not employee and not employee_cc:
                    raise UserError("There is no Email To and Email CC Configured !")
            ...
            if record.is_check_oem_warranty and record.child_ticket_id:
                ...
                # print("checkkkkkkkoem_warranty",child_ticket)
            ...
            if record.is_check_oem_repair_status and record.child_ticket_id:
                # print("checkkkkkkkoem_repair______warranty")
                # child_ticket = self.env['parent.ticket'].search([('id', '=', record.child_ticket_id.id)])
                stock_lot_obj = self.env['stock.lot'].search([('name', '=', record.child_ticket_id.stock_lot_id.name)])
                # repair
                repair_check_false = stock_lot_obj.filtered(lambda x: x.oem_repair_warranty_check == False)
