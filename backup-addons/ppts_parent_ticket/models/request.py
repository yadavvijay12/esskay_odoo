...
class Request(models.Model):
    ...
    # spare_requests_count = fields.Integer(compute='_count_total_spare_request')

    def spares_request_tickets(self):
        self.ensure_one()
        return {
            'name': 'Spare Requests',
            'view_mode': 'tree,form',
            'res_model': 'request.asset.line',
    ...
    # def _count_total_spare_request(self):
    #     for record in self:
    #         record.spare_requests_count = self.env['request.asset.line'].search_count([('child_ticket_id', '=', record.id)])
    ...
    def create(self, vals):
        ...
        amc = dict(res.parent_ticket_id._fields['amc_status'].selection).get(res.parent_ticket_id.amc_status) or dict(
            res.child_ticket_id._fields['amc_status'].selection).get(res.child_ticket_id.amc_status) or ''
        cmc = dict(res.parent_ticket_id._fields['cmc_status'].selection).get(res.parent_ticket_id.cmc_status) or dict(
        ...
        repair = dict(res.parent_ticket_id._fields['oem_repair_status'].selection).get(
            ...
            res.child_ticket_id._fields['oem_repair_status'].selection).get(res.child_ticket_id.oem_repair_status) or ''
        ...
        if not vals.get('is_spare_request'):
            ...
            if team:
                ...
                for rec in team_id.member_ids:
                    if rec.has_group('ppts_service_request.service_request_group_ct_user') and not rec.has_group(
                        ...
                        template_id.with_context(email_to=rec.partner_id.email, amc=amc, cmc=cmc, warranty=warranty,
                                                 repair=repair, extended=extended).sudo().send_mail(res.id,
                                                                                                    force_send=True)
        return res
