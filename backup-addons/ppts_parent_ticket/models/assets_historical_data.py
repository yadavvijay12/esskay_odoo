...
class AssetHistoricalData(models.Model):
    ...
    ref_ticket_id = fields.Many2one(comodel_name='child.ticket',string="Reference", ondelete='cascade', index=True, copy=False)
    ref_id = fields.Many2one(comodel_name='stock.lot',string="Assets Reference", ondelete='cascade', index=True, copy=False)
    # TICKET DETAIL
