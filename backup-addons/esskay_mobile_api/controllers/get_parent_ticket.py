...
class APIController(http.Controller):
    ...
    def _api_get_parent_ticket_value(self, **post,):
        ...
        if CT_Users:
            ...
            domain += [('assign_engineer_ids','in',user.id)]
        ...
        for rec in parent_ticket_ids:
            ...
            parent_ticket_list.append([id,name,parent_ticket_id_alias,call_source_id,call_date,service_category_id,
        ...
        if parent_ticket_list:
            return valid_response1(parent_ticket_list,count, 'parent ticket load successfully', 200)
        else:
            return valid_response1(parent_ticket_list,count, 'there is no parent ticket', 200)
    ...
    def api_parent_ticket_status_update_submit(self, **post):
        ...
        if post.get('add_latitude'):
            task_field_dic['add_latitude'] = str(post.get('add_latitude'))
        if post.get('add_longitude'):
            task_field_dic['add_longitude'] = str(post.get('add_longitude'))
        try:
            if parent_ticket_id:
