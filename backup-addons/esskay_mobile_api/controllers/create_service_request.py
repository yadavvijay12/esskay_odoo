...
class APIController(http.Controller):
    ...
    def _api_create_service_request(self, **post):
        ...
        request_type_ids = request.env['request.type'].sudo().search([('ticket_type','=',ticket_type),('code','=',request_type_code)])
        call_received_ids = request.env['call.received'].sudo().search([])
        team_ids = request.env['crm.team'].sudo().search([('is_service_request', '=', True)])
        ...
        custoemr_city_obj = request.env['customer.city'].sudo().search([])
        ...
        if ticket_type =='sr_installation':
            ...
            customer_name = {'key':'customer_name','type':service_req_obj._fields['customer_name'].type,'title':service_req_obj._fields['customer_name'].string,'required':service_req_obj._fields['customer_name'].required}
            ...
            customer_city_id = {'key':'customer_city_id','type':service_req_obj._fields['customer_city_id'].type,'title':service_req_obj._fields['customer_city_id'].string,
                           ...
                           'value':[{'id':field.id,'value':field.name,} for field in custoemr_city_obj]}
            ...
            reason = {'key':'reason','type':service_req_obj._fields['reason'].type,'title':service_req_obj._fields['reason'].string,'required':service_req_obj._fields['reason'].required}
            problem_reported = {'key':'problem_reported','type':service_req_obj._fields['problem_reported'].type,
                                'title':service_req_obj._fields['problem_reported'].string,'required':service_req_obj._fields['problem_reported'].required}
            ...
            service_list.append([service_request_id_alias,service_request_date,approver_id,child_ticket_type_id,
                                 customer_name,customer_email,customer_street,customer_street2,customer_city_id,
                                 customer_state_id,customer_zip,customer_country_id,install_addr_street,install_addr_street2,
                                 ...
                                 requested_by_name,requested_by_contact_number,call_received_id,team_id,
                                 ...
                                 remarks,survey_id,company_id,user_id,])
            ...
            #            'value': [{'id': field.id, 'value': field.name, } for field in user_ids]}
            service_list.append([service_request_id_alias, service_request_date, approver_id,
                                 call_source_id, service_type_id, service_category_id, request_type_id, reason,
                                 problem_reported, requested_by_name, requested_by_contact_number, call_received_id,
    ...
    def _api_create_service_request_submit(self, **post):
        ...
        try:
            ...
            if post.get('approver_id'):
                values_dic['approver_id'] = post.get('approver_id')
