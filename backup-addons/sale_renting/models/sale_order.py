...
class SaleOrder(models.Model):
    ...
    def sale_order_loaner_filter(self):
        ...
        sale_ids = self.env['sale.order'].search([('is_rental_order', '=',False),('state', 'not in', ('draft', 'sent', 'cancel'))])
        domain = "[('id', 'in', %s)]" % sale_ids.ids
        action['domain'] = domain
        return action
        ...
        # PT Configuration Notification
            if self.parent_ticket_id.parent_configuration_id and self.parent_ticket_id.parent_configuration_id.is_loaners:
                ...
                if self.parent_ticket_id.parent_configuration_id.alert_loaners == 'confirm' and self.rental_status == 'pickup':
                    ...
                    send = True
                elif self.parent_ticket_id.parent_configuration_id.alert_loaners == 'both' and self.rental_status in ['pickup', 'returned']:
                    send = True
                ...
                if team_alert_template_id and send:
                    teams = self.env['hr.employee'].search([('job_id', 'in',self.parent_ticket_id.parent_configuration_id.alert_loaners_approval_team_ids.ids)]).mapped('work_email')
                    email += ', '.join(teams)
        ...
        # CT Configuration Notification
            if self.child_ticket_id.child_configuration_id and self.child_ticket_id.child_configuration_id.is_loaners:
                ...
                if self.child_ticket_id.child_configuration_id.alert_loaners == 'confirm' and self.rental_status == 'pickup':
                    ...
                    send = True
                elif self.child_ticket_id.child_configuration_id.alert_loaners == 'both' and self.rental_status in ['pickup', 'returned']:
                    send = True
                ...
                if team_alert_template_id and send:
                    teams = self.env['hr.employee'].search([('job_id', 'in',self.child_ticket_id.child_configuration_id.alert_loaners_approval_team_ids.ids)]).mapped('work_email')
                    email += ', '.join(teams)
